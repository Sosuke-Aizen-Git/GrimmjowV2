...
async def search_posts(client, message):
    """Searches for posts in the saved channels."""
    if len(message.command) < 2:
        return await message.reply_text("Usage: `/search keyword`")
    
    query = " ".join(message.command[1:])
    chat_id = message.chat.id
    search_channels = get_search_channels()

    if not search_channels:
        return await message.reply_text("⚠ No channels set for searching. Admins can use `/setsearch`.")

    results = []
    for chat_id, username in search_channels.items():
        async for msg in client.search_messages(chat_id, query, limit=10):
            results.append(msg)

    if not results:
        return await message.reply_text("❌ No results found.")

    user_search_data[chat_id] = {"query": query, "results": results, "page": 0}
    await send_results(client, message, chat_id)

async def send_results(client, message, chat_id):
    """Sends search results with pagination."""
    data = user_search_data.get(chat_id)
    if not data:
        return
    
    page = data["page"]
    total_pages = max(1, (len(data["results"]) + 3) // 4)  # Ensure at least 1 page
    results = data["results"][page * 4 : (page + 1) * 4]

    buttons = [
        [InlineKeyboardButton(f"Post {i+1}", url=f"https://t.me/{msg.chat.username or msg.chat.id}/{msg.message_id}")]
    ...
    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton("⬅ Previous", callback_data=f"prev_{chat_id}"))
    nav_buttons.append(InlineKeyboardButton(f"📄 {page + 1}/{total_pages}", callback_data="page_info"))  # Page number button
    if (page + 1) * 4 < len(data["results"]):
        nav_buttons.append(InlineKeyboardButton("Next ➡", callback_data=f"next_{chat_id}"))

    if nav_buttons:
        buttons.append(nav_buttons)

    reply_markup = InlineKeyboardMarkup(buttons)
    await message.reply_text(f"🔍 **Search Results for:** `{data['query']}`", reply_markup=reply_markup)

@Client.on_callback_query(filters.regex(r"^(prev|next)_(\d+)$"))
async def paginate_results(client, query):
    """Handles pagination (next/previous buttons)."""
    ...
    if action == "prev":
        user_search_data[chat_id]["page"] -= 1
    elif action == "next":
        user_search_data[chat_id]["page"] += 1

    await query.message.delete()
    await send_results(client, query.message, chat_id)

@Client.on_callback_query(filters.regex(r"^page_info$"))
async def page_info_callback(client, query):
    """Handles the page number button (just shows an alert)."""
    await query.answer("This is the current page number.", show_alert=True)
